...
parser.add_argument(
    "--path",
    type=str,
    default=["/home/admin/workspace/user/improved-diffusion/sampled_images/cifar10_train.npz",
             "/home/admin/workspace/user/improved-diffusion/sampled_images/samples_50000x32x32x3.npz"],
...
def compute_statistics_of_path(path, model, batch_size, dims, device, resize=0):
    if path.endswith(".npz") or path.endswith(".npy"):
        ...
        # add another condition for npz file that try arr = f['arr_0'    
    else:
        # path_str = path[:]
        ...
        # save the statistics in .npz file with key 'mu' and 'sigma' and name as path_statistic.npz
        # np.savez("path_statistic.npz", mu=m, sigma=s)
    return m, s
...
def calculate_fid_given_paths(paths, batch_size, device, dims, resize=0):
    ...
    print(f"Calculating statistics for path: {paths[0]}")
    m1, s1 = compute_statistics_of_path(paths[0], model, batch_size, dims, device, resize)
    print(f"Calculating statistics for path: {paths[1]}")
